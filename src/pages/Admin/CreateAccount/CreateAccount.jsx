...
const CreateAccount = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    // Thêm classIds nếu là giáo viên hoặc học viên (optional)
    if (values.accountType === "teacher" || values.accountType === "student") {
      const selectedClassIds = values.classCodes
        ?.map(item => item?.code)
        ?.filter(Boolean) || [];
      selectedClassIds.forEach(id => {
    ...
    try {
      ...
      setIsModalVisible(true);
      form.resetFields();
      // Reset lại ô Mã lớp học đầu tiên trống (vẫn hiển thị)
      form.setFieldsValue({ classCodes: [{}] });
      message.success("Tạo tài khoản thành công!");
          ...
          <Col xs={24} lg={14}>
            <div className="form-container">
              <Form
                ...
                initialValues={{
                  // Hiển thị sẵn 1 ô chọn lớp (trống)
                  classCodes: [{}],
                }}
              >
                <Form.Item
                ...
                {/* Mã lớp học (optional) — hiện sẵn 1 ô, auto thêm ô mới khi chọn xong ô cuối */}
                <Form.List name="classCodes">
                  {(fields, { add, remove }) => (
                    <>
                      {fields.map(({ key, name, ...restField }, idx) => (
                        <Form.Item
                          key={key}
                          label={idx === 0 ? "Mã lớp học (không bắt buộc)" : undefined}
                        >
                          <Input.Group compact>
                            <Form.Item
                              {...restField}
                              name={[name, 'code']}
                              // KHÔNG required → optional
                              style={{ width: 'calc(100% - 32px)' }}
                            >
                              <Select
                                placeholder="Chọn mã lớp học"
                                ...
                                onChange={() => {
                                  // Nếu người dùng vừa chọn ở ô CUỐI CÙNG → tự động thêm 1 ô trống nữa
                                  const values = form.getFieldValue("classCodes") || [];
                                  const isLast = idx === fields.length - 1;
                                  if (isLast) {
                                    // Tránh thêm khi ô cuối chưa có gì (đã có onChange nghĩa là có chọn)
                                    add();
                                  }
                                }}
                                allowClear
                      ...
                      {/* Nút Thêm mã lớp học giữ nguyên (UI không đổi) */}
                      <Form.Item>
                        <Button
                          type="dashed"
                          onClick={() => add()}
