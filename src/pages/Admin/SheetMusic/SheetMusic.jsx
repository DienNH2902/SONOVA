...
const SheetMusic = () => {
  ...
  const handleAddSheet = async (values) => {
    try {
      // Step 1: Create the sheet music entry first
      ...
      if (response.ok) {
        const newSheetMusic = await response.json()
        const newSheetMusicId = newSheetMusic.sheetMusicId

        // Step 2: Upload sheet music files if provided
        if (values.sheetFiles && values.sheetFiles.length > 0) {
          const uploadPromises = values.sheetFiles.map(async (file) => {
            const sheetFormData = new FormData()
            sheetFormData.append("sheetMusicId", newSheetMusicId)
            sheetFormData.append("sheetFile", file.originFileObj)

            return fetch("https://innovus-api-f8ajdzdzhda0hxge.japanwest-01.azurewebsites.net/api/Sheet", {
              method: "POST",
              body: sheetFormData,
            })
          })
          ...
          if (failedUploads.length > 0) {
            message.warning(`Tạo bài nhạc thành công nhưng ${failedUploads.length} sheet nhạc không thể tải lên`)
          } else {
            message.success("Thêm sheet nhạc và các file thành công")
          }
        } else {
  ...
  const handleUpdateSheet = async (values) => {
    try {
      // Step 1: Update sheet music info
      const formData = new FormData()
      ...
      if (response.ok) {
        // Step 2: Upload new sheet music files if provided
        if (values.sheetFiles && values.sheetFiles.length > 0) {
          const uploadPromises = values.sheetFiles.map(async (file) => {
            const sheetFormData = new FormData()
            sheetFormData.append("sheetMusicId", selectedSheetMusic.sheetMusicId)
            sheetFormData.append("sheetFile", file.originFileObj)
          ...
          if (failedUploads.length > 0) {
            message.warning(
              `Cập nhật thông tin thành công nhưng ${failedUploads.length} sheet nhạc mới không thể tải lên`,
            )
          } else {
            message.success("Cập nhật sheet nhạc và các file thành công")
          }
        } else {
  ...
  const handleUpdateSongInfo = async (values) => {
    try {
      const formData = new FormData()
      formData.append("sheetMusicId", selectedSheetMusic.sheetMusicId)
      formData.append("musicName", values.musicName)
      ...
      if (response.ok) {
        message.success("Cập nhật thông tin bài nhạc thành công")
        await fetchData()
      } else {
        message.error("Không thể cập nhật thông tin bài nhạc")
      }
    } catch (error) {
      message.error("Lỗi khi cập nhật thông tin bài nhạc")
      console.error("Error updating song info:", error)
    }
  }

  const handleAddNewSheets = async (values) => {
    try {
      if (values.sheetFiles && values.sheetFiles.length > 0) {
        const uploadPromises = values.sheetFiles.map(async (file) => {
          const sheetFormData = new FormData()
        ...
        if (failedUploads.length > 0) {
          message.warning(`${failedUploads.length} sheet nhạc không thể tải lên`)
        } else {
          message.success("Thêm sheet nhạc mới thành công")
        }

        // Clear only the sheet files field
        editForm.setFieldsValue({ sheetFiles: [] })
        await fetchData()
      } else {
        message.warning("Vui lòng chọn ít nhất một file sheet nhạc để thêm")
      }
    } catch (error) {
      message.error("Lỗi khi thêm sheet nhạc mới")
      console.error("Error adding new sheets:", error)
    }
  }
      ...
      <div className="sheet-music-container-admin">
        ...
        >
          <Form form={addForm} layout="vertical" onFinish={handleAddSheet}>
            ...
            <Form.Item name="sheetFiles" label="File sheet nhạc" valuePropName="fileList" getValueFromEvent={normFile}>
              <Dragger {...{ ...uploadProps, multiple: true }}>
                <p className="ant-upload-drag-icon">
                  <InboxOutlined />
                </p>
                <p className="ant-upload-text">Nhấp hoặc kéo file vào khu vực này để tải lên</p>
                <p className="ant-upload-hint">Hỗ trợ tải lên nhiều file ảnh sheet nhạc</p>
              </Dragger>
            </Form.Item>
        ...
        >
          {selectedSheetMusic && (
            <div>
              <Title className="detail-modals-sheet" level={2}>
                {selectedSheetMusic.songName}
              </Title>

              {/* Song Information Section with Edit Form */}
              <Form form={editForm} layout="vertical">
                <div style={{ marginBottom: 24 }}>
                  <Row gutter={16}>
                    ...
                    <Col span={16} style={{ paddingLeft: 300 }}>
                      <Form.Item
                        name="musicName"
                        label="Tên bài hát"
                        rules={[{ required: true, message: "Vui lòng nhập tên bài hát" }]}
                      ...
                      {/* <p>
                        <strong className="detail-modal-sheet-title">Lượt yêu thích:</strong>{" "}
                        {selectedSheetMusic.favoriteCount}
                      </p> */}
                    </Col>
                  </Row>
                </div>

                {/* Add New Sheets Section */}
                <Form.Item
                  name="sheetFiles"
                  label="Thêm file sheet nhạc mới"
                  valuePropName="fileList"
                  getValueFromEvent={normFile}
                >
                  <Dragger {...{ ...uploadProps, multiple: true }}>
                    ...
                    <p className="ant-upload-text">Nhấp hoặc kéo file vào khu vực này để tải lên</p>
                    <p className="ant-upload-hint">Hỗ trợ tải lên nhiều file ảnh sheet nhạc mới</p>
                  </Dragger>
                </Form.Item>
                ...
                <Form.Item>
                  <Space>
                    <Button
                      ...
                      onClick={() => {
                        editForm
                          .validateFields(["musicName", "composer", "sheetQuantity", "genreIds", "coverImageFile"])
                          .then((values) => handleUpdateSongInfo(values))
                    ...
                    <Button
                      type="default"
                      onClick={() => {
                        const sheetFiles = editForm.getFieldValue("sheetFiles")
                        handleAddNewSheets({ sheetFiles })
                      }}
                    >
                      Thêm sheet nhạc
                    </Button>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <Title className="detail-modal-sheet-title" level={4}>
                  Danh sách Sheet Hiện Tại
                </Title>
              </div>
              ...
              <div style={{ marginBottom: 24 }}>
                <Row gutter={16}>
                  ...
                  <Col span={16} style={{ paddingLeft: 300, textAlign: "left" }}>
                    ...
                    <p>
                      <strong className="detail-modal-sheet-title">Số lượng sheet:</strong>{" "}
                      {selectedSheetMusic.quantity}
                    </p>
                    <p>
                      <strong className="detail-modal-sheet-title">Lượt yêu thích:</strong>{" "}
                      {selectedSheetMusic.favoriteCount}
        ...
        >
          <div style={{ marginBottom: 16, display: "flex", justifyContent: "flex-end" }}>
            <Button type="primary" icon={<PlusOutlined />} onClick={() => setIsAddGenreModalVisible(true)}>
              Tạo Thể loại mới
            </Button>
          </div>
          <Table
